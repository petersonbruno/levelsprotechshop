...
function waHomeLink() {
  const text = `Hello 👋%0AI'd like to inquire about products.`;
    ...
    <div className="min-h-screen bg-neutral-950 text-neutral-100 flex flex-col">
        ...
        {categories.map((category) => {
          ...
          return (
            <Link
              ...
              className={`rounded-2xl px-3 py-2 text-xs border flex flex-col items-center justify-center gap-1 ${isActive
      ...
      {!loading && !error && (
        <main className="flex-1 px-4 pb-24 pt-4">
          {activeCategory === "All" && query === "" && popularProducts.length > 0 && (
            <>
      ...
      <nav className="fixed bottom-0 inset-x-0 bg-neutral-950 border-t border-neutral-800">
        <div className="grid grid-cols-3 py-2 text-xs text-neutral-400">
          ...
          <Nav
            ...
            label="WhatsApp"
            active={true}
            onClick={() => window.open(waHomeLink(), "_blank")}
          />
          <Nav
            icon={<User />}
            label="Profile"
